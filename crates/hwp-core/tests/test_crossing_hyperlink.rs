...
fn test_issue144_crossing_hyperlink() {
    ...
    for (i, para) in section.paragraphs.iter().enumerate() {
        ...
        println!(
            "Control mask value: {:?}",
            para.para_header.control_mask.value
        );
                ...
                ParagraphRecord::CtrlHeader { data: ch_data } => {
                    ...
                    } = &ch_data.header.data
                    {
                        if field_type == "%hlk" || field_type == "hlk" {
                            println!(
                                "Hyperlink CtrlHeader: field_type={}, command={}",
                                field_type, command
                            );
                        }
                    }
    ...
    // Assertions
    assert!(
        markdown.contains("[google google google google google google](http"),
    ...
    assert!(
        markdown.contains("gmail.com"),
        "Should contain gmail.com hyperlink"
    );
}
