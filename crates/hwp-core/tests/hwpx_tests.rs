...
fn test_hwpx_markdown_snapshots() {
    ...
    if hwpx_files.is_empty() {
        println!("No HWPX files found, skipping test");
        return;
    }
...
fn test_hwpx_specific_file() {
    // Test specific HWPX fixture file
    if let Some(file_path) = find_hwpx_fixture_file("test-hwpx.hwpx") {
        let data = std::fs::read(&file_path).expect("Failed to read HWPX file");
        let parser = HwpParser::new();
        let document = parser.parse(&data).expect("Failed to parse HWPX");
        ...
        println!("test-hwpx.hwpx not found, skipping");
    }
}
